...
export default function App() {
  ...
  if (viewMode === 'admin' && currentSession.type === 'admin' && currentSession.adminUser) {
    return (
      <AdminDashboard
        ...
        onPromotionsUpdated={(updated) => setPromotions(updated)}
      />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white transition-colors duration-200 flex flex-col font-sans">
      ...
      {/* 3. Featured Products Carousel (Destaques) */}
      <FeaturedCarousel
        products={products.filter((p) => p.isFeatured)}
        b2bUser={b2bUser}
        onOpenDetails={(p) => setSelectedProduct(p)}
        onConsultWhatsApp={(msg, product) => handleConsultWhatsApp(msg, product)}
      />

      {/* 4. Seasonal & Themed Promotions (Promoções) */}
      <PromotionsSection
        promotions={promotions}
        products={products}
        onConsultWhatsApp={(msg) => handleConsultWhatsApp(msg)}
      />
      ...
      {/* 6. Instagram Social Proof Showcase */}
      <InstagramFeed />
